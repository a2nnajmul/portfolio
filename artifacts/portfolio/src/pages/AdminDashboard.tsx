...
type ContentSubTab = "hero" | "skills" | "about-tabs" | "contact";
...
function ContactEditor() {
  const [data, setData] = useState<ContactContent>({ heading: "", email: "", phone: "", location: "", formHeading: "" });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const [error, setError] = useState("");
  ...
  useEffect(() => {
    adminFetch<ContactContent>("/admin/content/contact").then(setData).catch(() => setError("Failed to load")).finally(() => setLoading(false));
  }, []);
  ...
  async function save(e: FormEvent) {
    ...
    setSaving(true); setError(""); setSaved(false);
    try {
      const updated = await adminFetch<ContactContent>("/admin/content/contact", { method: "PUT", body: JSON.stringify(data) });
      setData(updated); setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    } catch { setError("Save failed"); }
    finally { setSaving(false); }
  ...
  return (
    <div className="max-w-2xl">
      {error && <p role="alert" className="mb-4 text-sm text-destructive bg-destructive/10 px-4 py-2 rounded-lg">{error}</p>}
      <form onSubmit={save} className="bg-card border border-border rounded-2xl p-6 space-y-4">
        <Field label="Section Heading" value={data.heading} onChange={(v) => setData((d) => ({ ...d, heading: v }))} placeholder="Get In Touch" />
        <Field label="Email" value={data.email} onChange={(v) => setData((d) => ({ ...d, email: v }))} placeholder="your@email.com" />
        <Field label="Phone" value={data.phone} onChange={(v) => setData((d) => ({ ...d, phone: v }))} placeholder="(+880) 1234567890" />
        <Field label="Location" value={data.location} onChange={(v) => setData((d) => ({ ...d, location: v }))} multiline placeholder="City, Country" />
        <Field label="Form Heading" value={data.formHeading} onChange={(v) => setData((d) => ({ ...d, formHeading: v }))} placeholder="Send Me a Message" />
        <div className="flex items-center gap-4 pt-2">
          <button type="submit" disabled={saving} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 disabled:opacity-60 transition">
...
const AD_POSITIONS = [
  ...
  { value: "before-recommended", label: "Before Recommended" },
];
...
function SettingsTab() {
  ...
  const [adSlots, setAdSlots] = useState<AdminAdSlot[]>([]);
  ...
  useEffect(() => {
    adminFetch<{ enabled: boolean; headScript: string; adSlots: AdminAdSlot[] }>("/admin/settings/ads")
      .then((d) => { setAdsEnabled(d.enabled); setHeadScript(d.headScript); setAdSlots(d.adSlots || []); })
      .catch(() => {})
      .finally(() => setAdsLoading(false));
  }, []);

  function addAdSlot() {
    setAdSlots((prev) => [...prev, { id: crypto.randomUUID(), label: "", position: "after-header", adUnitCode: "" }]);
  }
  ...
  function updateAdSlot(id: string, field: keyof AdminAdSlot, value: string) {
  ...
  async function saveAds(e: FormEvent) {
    ...
    try {
      const updated = await adminFetch<{ enabled: boolean; headScript: string; adSlots: AdminAdSlot[] }>(
        "/admin/settings/ads",
        { method: "PUT", body: JSON.stringify({ enabled: adsEnabled, headScript, adSlots }) }
      );
      setAdsEnabled(updated.enabled);
      setHeadScript(updated.headScript);
      setAdSlots(updated.adSlots || []);
      setAdsSaved(true);
        ...
        {adsLoading ? <p className="text-muted-foreground text-sm">Loading…</p> : (
          <form onSubmit={saveAds} className="space-y-5">
            ...
            <div className="border-t border-border pt-5">
              <div className="flex items-center justify-between mb-4">
                <label className="block text-sm font-medium text-foreground">Ad Slots</label>
                <button
                  type="button"
                  onClick={addAdSlot}
              ...
              {adSlots.length === 0 ? (
                <div className="bg-muted/50 rounded-xl p-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">No ad slots configured. Click "Add Slot" to create one.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {adSlots.map((slot, idx) => (
                    <div key={slot.id} className="bg-muted/30 rounded-xl p-4 space-y-3 border border-border/50">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-muted-foreground uppercase">Slot {idx + 1}</span>
                        <button
                          type="button"
                          onClick={() => removeAdSlot(slot.id)}
                          className="flex items-center gap-1 px-2 py-1 rounded-lg text-destructive text-xs hover:bg-destructive/10 transition"
                        >
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition"
                          />
                        </div>
                        <div>
                      ...
                      <div>
                        <label className="block text-xs font-medium text-foreground mb-1">Ad Unit Code</label>
                        <textarea
                          value={slot.adUnitCode}
                          onChange={(e) => updateAdSlot(slot.id, "adUnitCode", e.target.value)}
                          rows={3}
                          placeholder={'<ins class="adsbygoogle" ...></ins>\n<script>(adsbygoogle = window.adsbygoogle || []).push({});</script>'}
                          className="w-full px-3 py-2 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 transition resize-y font-mono text-xs"
                        />
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-muted/50 rounded-xl p-4 text-xs text-muted-foreground space-y-1">
              <p className="font-medium text-foreground text-sm">Ad Placement Positions</p>
              <p><strong>After Header</strong> — Below the post title, tags, and description</p>
              <p><strong>Mid Content</strong> — After the main article content</p>
              <p><strong>Before Recommended</strong> — Before the recommended posts section</p>
            </div>
